...
const getMediaQuality = (releaseDate?: string) => {
  if (!releaseDate) return "Unknown Quality";
  ...
  if (release > oneYearAgo) return "Cam Quality";
  return "HD";
};

export default function CarousalCard(props: CarousalCardProps) {
  const { show, isDetailsPage, type } = props;
  const releaseDate = props.show.release_date || props.show.first_air_date;
  const mediaQuality = getMediaQuality(releaseDate);
          ...
          <div className="flex md:hidden h-[70vh] relative">
            ...
            <div className="border-white absolute flex justify-between bg-gradient-to-t from-background to-transparent bottom-0 top-1/2 w-full flex-col">
              ...
              <div className="flex items-center flex-col">
                ...
                <div className="text-xs opacity-50">{mediaQuality}</div>
